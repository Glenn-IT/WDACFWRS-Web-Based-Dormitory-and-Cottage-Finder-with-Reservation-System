document.addEventListener("DOMContentLoaded", () => {
  ...
  async function render() {
    ...
    const data = await DataAPI.getUsers({ search, status, sortBy });
    users = data.users || [];
  ...
  function viewUser(id) {
    ...
    document.getElementById("user-view-body").innerHTML = `
      <div class="text-center mb-3">
        <img src="${resolveAsset(u.profilePic)}" class="rounded-circle mb-2" style="width:96px;height:96px;object-fit:cover;">
  ...
  async function setStatus(id, status) {
    ...
    await withLoading(async () => {
      try {
        await DataAPI.setUserStatus(id, status);
        await render();
